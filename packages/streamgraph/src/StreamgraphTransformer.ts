...
export class StreamgraphTransformer {
    public static transform(categorical: DataViewCategorical): StreamgraphData {
        ...
        // Build stacked data structure: yValue -> xValue -> value
        const stackedData = new Map<string, Map<string, number>>();
        yValues.forEach(y => {
            stackedData.set(y, new Map<string, number>());
            xValues.forEach(x => {
                stackedData.get(y)!.set(x, 0);
            });
        });

        dataPoints.forEach(dp => {
            const yMap = stackedData.get(dp.yValue);
            if (yMap) {
                yMap.set(dp.xValue, dp.value);
            }
        });
